...
class Trainer(object):  
    ...
    def save_softmax_output(self, outputs, ways):
        path = os.path.join(self.params.dump_path, '%s.pth' % ways)
...
class EncDecTrainer(Trainer):
    ...
    def mt_step(self, lang1, lang2, lambda_coeff, big_trainer):
        ...
        # 这里需要注意, 这个mask矩阵是类似与 decoder 的mask矩阵, 但是最后一个目标单词不预测
    ...
    def bt_step(self, lang1, lang2, lang3, lambda_coeff):
        ...
        # loss
        scores, loss = self.decoder('predict', tensor=dec3, pred_mask=pred_mask, y=y1, get_scores=True)
