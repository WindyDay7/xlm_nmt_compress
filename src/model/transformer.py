...
def get_masks(slen, lengths, causal):
    ...
    return mask.bool(), attn_mask.bool()
...
class PredLayer(nn.Module):
    ...
    def __init__(self, params):
        ...
        self.if_diff = params.distillation
        dim = params.emb_dim
...
class TransformerModel(nn.Module):
    ...
    def fwd(self, x, lengths, causal, src_enc=None, src_len=None, positions=None, langs=None, cache=None):
        ...
        # generate masks
        # 这个mask矩阵和传统的mask矩阵是一样的,只是实现过程不一样
        # 下面这三个参数最重要,分别是,句子最长长度, 每个句子的长度, 以及是否是 Decoder, 其中lengths最重要
        ...
        # embeddings
        # 最初的tensor
        # x 的输入维度为 slen, bs, transpose之后为 bs, slen, 然后进行 embedding 之后为 bs, slen, D_model
        ...
        # 将最初的tensor乘以mask矩阵, 注意要将 mask 增加一个维度, 因为tensor维度比 mask多了
        ...
        for i in range(self.n_layers):
            ...
            # 这里是完全符合 Attention is all you need 原文的方式构建 encoder 与 decoder
            ...
            # 为什么每次都要进行一次mask, 因为经过一次输出可能将输入 tensor中的 0 变成大于 0的值
            tensor *= mask.unsqueeze(-1).to(tensor.dtype)
        ...
        # move back sequence length to dimension 0, 将 batch_size 与 Length 换回去
